   ...
   public:
    ...
    buffer_c16_t execute(const buffer_c16_t& src, const buffer_c16_t& dst) {
        ...
        for (size_t i = 0; i < src.count; ++i) {
            ...
            const int32_t sine_first = sine_q15_[index];
            const int32_t cosine_first = sine_q15_[cosine_index];
            const int32_t sine =
                sine_first +
                (((sine_q15_[next] - sine_first) * fraction) >> 8);
            const int32_t cosine =
                cosine_first +
                (((sine_q15_[cosine_next] - cosine_first) * fraction) >> 8);
             ...
             * (I+jQ) * (cos-j sin). */
            const int32_t out_i = __SMUAD(sample, oscillator) >> 15;
            const int32_t out_q = __SMUSDX(oscillator, sample) >> 15;
   ...
   private:
    ...
    void update_taps() {
        ...
        for (size_t i = 0; i < taps_count; ++i) {
            ...
            complex_taps_[i] = {
                static_cast<int16_t>((tap * cosine) >> 14),
                static_cast<int16_t>((tap * sine) >> 14)};
